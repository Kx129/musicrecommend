...
def playView(request, song_id):
    ...
    if current_user.username != '':
        records = UserSongRecord.objects.all().filter(song_id=song_id, user_id=current_user.id)
        if records:
            record = records[0]
            record.num = record.num + 1
            record.save()
        else:
            record = UserSongRecord.objects.create(song_id=song_id,user_id=current_user.id,num=1)
        ...
        ListenRecord.objects.create(user_id=current_user.id,song_id=song_id,listen_date=now)
    ...
    # 歌词
    if str(song_info.song_lyrics.name) != '暂无歌词':
        f = open('' +song_info.song_lyrics.name, 'r', encoding='utf-8')
        song_lyrics = f.read()
        f.close()
    # 相关歌曲
